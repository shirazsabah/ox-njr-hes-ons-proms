# R NHSD 12 Join Primary hip replacements-ONS-HES-PROMs
...
# # Describe
# desc <- dbGetQuery(con, "DESCRIBE;")

########################################## Merge ph to ONS ##########################################

# Keep the first ph for a given patient
dbExecute(con, "CREATE TABLE ph_first AS
          WITH cte AS
          (
            SELECT *,
...
# Merge
dbExecute(con, "CREATE TABLE ph_ons AS
                SELECT ph_first.*, ons.*
                FROM ph_first
                LEFT OUTER JOIN ons
                ON ons.nn_nid = ph_first.nn_nid;")
      
# Exclude records where DOD < DOR
dbExecute(con, "DELETE FROM ph_ons WHERE dod < primary_op_date;")
...
dbExecute(con, "ALTER TABLE ph_ons ALTER nn_nid TYPE VARCHAR;")

# Create fields 'start' and 'end' to provides a tolerance of +/- 1 week for an episode to merge
dbExecute(con, "ALTER TABLE ph_ons ADD COLUMN start_date DATE;")
dbExecute(con, "UPDATE ph_ons SET start_date = primary_op_date - 7;")

dbExecute(con, "ALTER TABLE ph_ons ADD COLUMN end_date DATE;")
dbExecute(con, "UPDATE ph_ons SET end_date = primary_op_date + 7;")

# Merge to HES

dbExecute(con, "CREATE TABLE temp AS
                  SELECT ph_ons.*, hes_working.*
                  FROM ph_ons
                  LEFT OUTER JOIN hes_working
                  ON hes_working.study_id = ph_ons.nn_nid
                  AND hes_working.admidate_filled >= ph_ons.start_date
                  AND hes_working.admidate_filled <= ph_ons.end_date;")
...
dbExecute(con, "CREATE TABLE ph_ons_hes AS
                WITH cte AS
                (
                   SELECT *,
...
dbExecute(con, "ALTER TABLE ph_ons_hes DROP row_num")
...
dbExecute(con, "CREATE TABLE temp3 AS
          WITH cte AS 
            ...
            WHERE EXISTS (
              ...
              FROM ph_ons_hes
              WHERE ph_ons_hes.epikey = proms.epikey
                AND ph_ons_hes.primary_op_date - proms.q1_completed_date <=90
                AND ph_ons_hes.primary_op_date - proms.q1_completed_date >=0
                AND proms.q2_completed_date - ph_ons_hes.primary_op_date >=180
                AND proms.q2_completed_date - ph_ons_hes.primary_op_date <=365
                AND proms.q1_completed_date IS NOT NULL
                AND proms.q2_completed_date IS NOT NULL
                )
            )

          SELECT ph_ons_hes.*, cte.*
          FROM ph_ons_hes
          LEFT OUTER JOIN cte
          ON cte.epikey = ph_ons_hes.epikey
            AND cte.rn = 1;")
...
## Write Duck DB table
dbWriteTable(con, "ph_ons_hes_proms", df)
...
# Number of ph supplied by NJR
dbGetQuery(con, "SELECT count(*)
           FROM ph;")

# Number of first ph for a given patient
dbGetQuery(con, "SELECT count(*)
           FROM ph_first;")
...
# Number with a returned PROM within an appropriate date where OHS and success transition complete
dbGetQuery(con, "SELECT count(*)
           FROM ph_ons_hes_proms
           WHERE proms_serial_no IS NOT NULL
            AND cca = 1;")

# Number by procedure type with a returned PROM within an appropriate date
