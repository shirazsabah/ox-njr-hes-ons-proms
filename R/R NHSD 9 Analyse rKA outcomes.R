...
# Outcome measures
  #1 Implant survivorship
        ## a. Logistic regression at 2y (not used here; set up for rKR caseload study; will eventually need filtering from 1/1/2009 to 31/12/2017;
        ## b. Survival analysis
...
##### a. Logistic regression at 2y
  ## Create an outcome variable for first rKR procedures, by reshaping second revision procedures from long to wide
  ## Mortality is ignored and follow-up is from date of revision to date of re-revision or end of study
    ### nn_nid, side --> identify the knee
  ...
  ## Note: 
    ### This uses the dataset 'rk', which includes all rKR on the NJR
    ### i.e. Second, third, fourth, etc.
    ### Follow-up is to 31/12/2019, hence year %between% c("2009", "2019")

# Bring all rKR into R environment
rk <- dbGetQuery(con, "SELECT * FROM all_rk")
...
# However, there are some (likely) duplicates when time to second rKR = 0 days
first_is <-
...
setwd(paste0(data_dir,"FIGS/"))

rfr <- c("Infection","Malalignment","Loosening/Lysis","Instability",
  "Fracture","Progressive Arthritis", "Stiffness", "Unexplained pain", "Other")

for (val in rfr) {
  
  KMfilt <- first_is %>% filter(ifrhier2 == val)
...
# p_load(broom)
# survfit(Surv(fu, outcome_sa) ~factor(ifrhier2), data=first_is) %>% tidy()
...
survtbl$strata <- str_remove(survtbl$strata, '^factor\\(ifrhier2\\)=') ### Need to escape brackets with \\ for them to be removed
...
## First rKR recruited until 30/06/2019

t1 <- first_is %>% select(age_at_revision, gender, asa_grade, charl01, bmi_plaus, ethnic5, imd5, year, ifrhier2) %>% droplevels()
...
tbl1 <- 
  tbl_summary(
    t1,
    by = "ifrhier2", # split table by group
    missing = "no",
...
# mort90d <-
#   first_is %>% tabyl(mort90, ifrhier2) %>%
#   adorn_percentages("col") %>%
#   adorn_pct_formatting(digits = 2) %>%
#   adorn_ns()
# 
...
t2 = first_is %>% select("mort90", "ifrhier2")
...
comps <-
  first_is %>% 
  select(EPIKEY, ifrhier2, compacva90, complrti90, compmi90, compdvt90, comppe90, compaki90, computi90) %>% 
  mutate(any_med_comp =
...
# This determines the order of the table
t3 = comps %>% select("any_med_comp", "compaki90", "compdvt90", "complrti90", "compmi90", "comppe90", "compacva90", "computi90", "ifrhier2")
...
los <-
  first_is %>%
  select(EPIDUR, SPELDUR, los, ifrhier2, EPIKEY) %>% 
...
x <-
  ...
  select(ifrhier2, `Length of stay (days) / median (Q1 - Q3)`) %>% 
  tibble::rownames_to_column() %>%  
  pivot_longer(-rowname) %>% 
  pivot_wider(names_from=rowname, values_from=value) %>% 
  filter(!name =="ifrhier2")
...
# Select variables
t4 = proms %>% filter(!is.na(proms_serial_no)) %>% select("kr_q1_score", "kr_q2_score", "ch", "mic01", "q1_eq5d_index", "q2_eq5d_index", "eq5d_index_change", "q2_satisfaction", "q2_success", "ifrhier2") %>% droplevels()
...
t5 = proms %>% filter(!is.na(proms_serial_no)) %>% select("sat", "suc", "ifrhier2") %>% droplevels()
...
t6 <- first_is %>% select("promyn", "ifrhier2")
...
save.image(paste0(data_dir,"R_IMAGES/multimodal_rkr.RData"))
