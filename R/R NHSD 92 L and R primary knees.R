# R NHSD - Linking both right and left sided procedures from NJR to HES-ONS-PROMs
...
########################################## Right- and left-sided primary knees ##########################################

dbExecute(con, "CREATE TABLE pk_right AS
          WITH cte AS
          (
            SELECT *,
...
dbExecute(con, "CREATE TABLE pk_left AS
          WITH cte AS
          (
            SELECT *,
            ROW_NUMBER() OVER (PARTITION BY nn_nid ORDER BY primary_op_date ASC) AS rn
...
# Merge right
dbExecute(con, "CREATE TABLE pk_right_ons AS
                SELECT pk_right.*, ons.*
                FROM pk_right
                LEFT OUTER JOIN ons
                ON ons.nn_nid = pk_right.nn_nid;")

# Exclude records where DOD < DOR
dbExecute(con, "DELETE FROM pk_right_ons WHERE dod < primary_op_date;")


# Merge left
dbExecute(con, "CREATE TABLE pk_left_ons AS
                SELECT pk_left.*, ons.*
                FROM pk_left
                LEFT OUTER JOIN ons
                ON ons.nn_nid = pk_left.nn_nid;")

# Exclude records where DOD < DOR
dbExecute(con, "DELETE FROM pk_left_ons WHERE dod < primary_op_date;")
...
dbExecute(con, "ALTER TABLE pk_right_ons ALTER nn_nid TYPE VARCHAR;")
dbExecute(con, "ALTER TABLE pk_left_ons ALTER nn_nid TYPE VARCHAR;")

# Create fields 'start' and 'end' to provides a tolerance of +/- 1 week for an episode to merge
dbExecute(con, "ALTER TABLE pk_right_ons ADD COLUMN start_date DATE;")
dbExecute(con, "UPDATE pk_right_ons SET start_date = primary_op_date - 7;")
dbExecute(con, "ALTER TABLE pk_right_ons ADD COLUMN end_date DATE;")
dbExecute(con, "UPDATE pk_right_ons SET end_date = primary_op_date + 7;")

dbExecute(con, "ALTER TABLE pk_left_ons ADD COLUMN start_date DATE;")
dbExecute(con, "UPDATE pk_left_ons SET start_date = primary_op_date - 7;")
dbExecute(con, "ALTER TABLE pk_left_ons ADD COLUMN end_date DATE;")
dbExecute(con, "UPDATE pk_left_ons SET end_date = primary_op_date + 7;")

# Merge to HES

dbExecute(con, "CREATE TABLE temp_right AS
                  SELECT pk_right_ons.*, hes_working.*
                  FROM pk_right_ons
                  LEFT OUTER JOIN hes_working
                  ON hes_working.study_id = pk_right_ons.nn_nid
                  AND hes_working.admidate_filled >= pk_right_ons.start_date
                  AND hes_working.admidate_filled <= pk_right_ons.end_date;")

dbExecute(con, "CREATE TABLE temp_left AS
                  SELECT pk_left_ons.*, hes_working.*
                  FROM pk_left_ons
                  LEFT OUTER JOIN hes_working
                  ON hes_working.study_id = pk_left_ons.nn_nid
                  AND hes_working.admidate_filled >= pk_left_ons.start_date
                  AND hes_working.admidate_filled <= pk_left_ons.end_date;")
...
dbExecute(con, "CREATE TABLE pk_right_ons_hes AS
                WITH cte AS
                (
                   SELECT *,
...
dbExecute(con, "ALTER TABLE pk_right_ons_hes DROP row_num")
...
dbExecute(con, "CREATE TABLE pk_left_ons_hes AS
                WITH cte AS
                (
                   SELECT *,
                         ROW_NUMBER() OVER (PARTITION BY nn_nid ORDER BY date_diff ASC) AS row_num
...
# Remove the row_number field
dbExecute(con, "ALTER TABLE pk_left_ons_hes DROP row_num")
...
dbExecute(con, "CREATE TABLE temp3_right AS
          WITH cte AS 
            ...
            WHERE EXISTS (
              ...
              FROM pk_right_ons_hes
              WHERE pk_right_ons_hes.epikey = proms.epikey
                AND pk_right_ons_hes.primary_op_date - proms.q1_completed_date <=90
                AND pk_right_ons_hes.primary_op_date - proms.q1_completed_date >=0
                AND proms.q2_completed_date - pk_right_ons_hes.primary_op_date >=180
                AND proms.q2_completed_date - pk_right_ons_hes.primary_op_date <=365
                AND proms.q1_completed_date IS NOT NULL
                AND proms.q2_completed_date IS NOT NULL
                )
            )

          SELECT pk_right_ons_hes.*, cte.*
          FROM pk_right_ons_hes
          LEFT OUTER JOIN cte
          ON cte.epikey = pk_right_ons_hes.epikey
            AND cte.rn = 1;")
...
dbExecute(con, "CREATE TABLE temp3_left AS
          WITH cte AS 
            ...
            WHERE EXISTS (
              SELECT epikey, primary_op_date
              FROM pk_left_ons_hes
              WHERE pk_left_ons_hes.epikey = proms.epikey
                AND pk_left_ons_hes.primary_op_date - proms.q1_completed_date <=90
                AND pk_left_ons_hes.primary_op_date - proms.q1_completed_date >=0
                AND proms.q2_completed_date - pk_left_ons_hes.primary_op_date >=180
                AND proms.q2_completed_date - pk_left_ons_hes.primary_op_date <=365
                AND proms.q1_completed_date IS NOT NULL
                AND proms.q2_completed_date IS NOT NULL
                )
            )

          SELECT pk_left_ons_hes.*, cte.*
          FROM pk_left_ons_hes
          LEFT OUTER JOIN cte
          ON cte.epikey = pk_left_ons_hes.epikey
            AND cte.rn = 1;")
...
## Write Duck DB table
dbWriteTable(con, "pk_right_ons_hes_proms", df)
...
## Write Duck DB table
dbWriteTable(con, "pk_left_ons_hes_proms", df)
...
dbExecute(con, "CREATE TABLE pk_combined AS
          SELECT *
  FROM pk_left_ons_hes_proms

 UNION

 SELECT *
   FROM pk_right_ons_hes_proms
          ")


## Remove intermediary tables
dbExecute(con, "DROP TABLE pk_left")
dbExecute(con, "DROP TABLE pk_left_ons")
dbExecute(con, "DROP TABLE pk_left_ons_hes")
dbExecute(con, "DROP TABLE pk_left_ons_hes_proms")
dbExecute(con, "DROP TABLE pk_right")
dbExecute(con, "DROP TABLE pk_right_ons")
dbExecute(con, "DROP TABLE pk_right_ons_hes")
dbExecute(con, "DROP TABLE pk_right_ons_hes_proms")
