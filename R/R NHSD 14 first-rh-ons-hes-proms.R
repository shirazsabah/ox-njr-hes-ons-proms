# R NHSD 8 Join First rHR-ONS-HES-PROMs
...
########################################## Merge first rh to ONS ##########################################

# Note: These are already the 'first' first_rh for a given patient

dbExecute(con, "CREATE TABLE rh_ons AS
                SELECT first_rh.*, ons.*
                FROM first_rh
                LEFT OUTER JOIN ons
                ON ons.nn_nid = first_rh.nn_nid;")

# Exclude records where DOD < DOR
dbExecute(con, "DELETE FROM rh_ons WHERE dod < op_date;")
...
dbExecute(con, "CREATE TABLE rh_ons_hes AS
                WITH cte AS
                (
                   SELECT *,
...
dbExecute(con, "ALTER TABLE rh_ons_hes DROP row_num")
...
dbExecute(con, "CREATE TABLE temp3 AS
          WITH cte AS (
            ...
            WHERE EXISTS (
              ...
              FROM rh_ons_hes
              WHERE rh_ons_hes.epikey = proms.epikey
                AND rh_ons_hes.op_date - proms.q1_completed_date <=90
                AND rh_ons_hes.op_date - proms.q1_completed_date >=0
                AND proms.q2_completed_date - rh_ons_hes.op_date >=180
                AND proms.q2_completed_date - rh_ons_hes.op_date <=365
                AND proms.q1_completed_date IS NOT NULL
                AND proms.q2_completed_date IS NOT NULL
                )
          )

          SELECT rh_ons_hes.*, cte.*
          FROM rh_ons_hes
          LEFT OUTER JOIN cte
          ON cte.epikey = rh_ons_hes.epikey
            AND cte.rn = 1;")
...
## Write Duck DB table
dbWriteTable(con, "rh_ons_hes_proms", df, overwrite=TRUE)
