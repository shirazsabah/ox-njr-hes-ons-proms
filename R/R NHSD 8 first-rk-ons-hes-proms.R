# R NHSD 8 Join First rKR-ONS-HES-PROMs
...
dbExecute(con, "CREATE TABLE rk_ons AS
                SELECT first_rk.*, ons.*
                FROM first_rk
                LEFT OUTER JOIN ons
                ON ons.nn_nid = first_rk.nn_nid;")

# Exclude records where DOD < DOR
dbExecute(con, "DELETE FROM rk_ons WHERE dod < op_date;")
...
dbExecute(con, "ALTER TABLE rk_ons ALTER nn_nid TYPE VARCHAR;")

# Merge to HES

# Merge to first rKR
dbExecute(con, "CREATE TABLE temp AS
                  SELECT rk_ons.*, hes_working.*
                  FROM rk_ons
                  LEFT OUTER JOIN hes_working
                  ON hes_working.study_id = rk_ons.nn_nid
                  AND hes_working.admidate_filled >= rk_ons.start
                  AND hes_working.admidate_filled <= rk_ons.end;")
...
dbExecute(con, "CREATE TABLE rk_ons_hes AS
                WITH cte AS
                (
                   SELECT *,
...
dbExecute(con, "ALTER TABLE rk_ons_hes DROP row_num")
...
dbExecute(con, "CREATE TABLE temp3 AS
          WITH cte AS (
            ...
            WHERE EXISTS (
              ...
              FROM rk_ons_hes
              WHERE rk_ons_hes.epikey = proms.epikey
                AND rk_ons_hes.op_date - proms.q1_completed_date <=90
                AND rk_ons_hes.op_date - proms.q1_completed_date >=0
                AND proms.q2_completed_date - rk_ons_hes.op_date >=180
                AND proms.q2_completed_date - rk_ons_hes.op_date <=365
          ...
          FROM rk_ons_hes
          LEFT OUTER JOIN cte
          ON cte.epikey = rk_ons_hes.epikey
            AND cte.rn = 1;")
...
## Write Duck DB table
dbWriteTable(con, "rk_ons_hes_proms", df, overwrite=TRUE)
